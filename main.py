...
# Random play minimum reward 0.0
# Random play mean reward 1.4
# Random play max reward 8.0
...
def train(config):
  ...
  with tf.name_scope("conv1"):
    ...
    r_conv1 = conv2d(norm_state, W_conv1, 2) + b_conv1
    n_conv1 = tf.contrib.layers.layer_norm(r_conv1)
    h_conv1 = tf.maximum(n_conv1, 0.1 * n_conv1)
  ...
  with tf.name_scope("conv2"):
    ...
    h_conv2 = tf.maximum(n_conv2, 0.1 * n_conv2)
  ...
  with tf.name_scope("conv3"):
    ...
    h_conv3 = tf.maximum(n_conv3, 0.1 * n_conv3)
  ...
  with tf.name_scope("hidden"):
    ...
    h_hidden = tf.maximum(n_hidden, 0.1 * n_hidden)
  ...
  for episode in range(NUMBER_OF_EPISODES):
    ...
    for step in range(MAX_STEPS):
      ...
      if total_steps > MINIMUM_SAMPLE_SIZE:
        ...
        # # Save model
        # if total_steps % 1000 == 0:
        #   save_path = model_saver.save(session, MODEL_PATH)
